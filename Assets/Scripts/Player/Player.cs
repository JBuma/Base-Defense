...
public class Player : MonoBehaviour {
	...
	private void handleAnimation() {
		...
		if (Mathf.Abs(horizontalThrow) > Mathf.Epsilon) {
			...
			if (Input.GetKeyDown(KeyCode.S) && colliderFeet.IsTouchingLayers(LayerMask.GetMask("Ground"))) {
		...
		if (transform.InverseTransformDirection(rigidbody.velocity).y < 0) {
			animator.SetBool("Falling", true);
		}
		if (colliderFeet.IsTouchingLayers(LayerMask.GetMask("Ground"))) {
			animator.SetBool("Jumping", false);
			animator.SetBool("Falling", false);
		}
	}
